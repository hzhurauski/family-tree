...
export function NextItem(currentTarget) {
  ...
  if (nextItem.nextElementSibling == null) {
    ...
    $(AfterNextItem)[0].style.left =
      GetNumber(nextItem.style.left) + space + 'px'
  } else {
  ...
  if (LengthSlider == 3) {
    ...
    if (AfterNextItem.nextElementSibling == null) {
      ...
      $(AfterAfterNextItem)[0].style.left =
        GetNumber(AfterNextItem.style.left) + space + 'px'
    } else {
...
export function GetSpace(currentTarget) {
  SliderBlock = $('#' + $(currentTarget.parentElement)[0].id + ' .PrevItem')[0]
    .nextElementSibling
  LengthSlider =
    SliderBlock.firstElementChild.nextElementSibling.nextElementSibling.value -
    0
