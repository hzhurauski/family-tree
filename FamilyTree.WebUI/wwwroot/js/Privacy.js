...
function OnPrivacyLevelButtonClick(event) {
  let privacyLevelValue = $(event.currentTarget).find('input').val()
...
function OnLimitTypeButtonClick(event) {
  let limitTypeValue = $(event.currentTarget).find('input').val()
...
function LoadPrivacyData(privacy) {
  ...
  if (privacy == null) {
    privacyModal.find('.privacy-confidential').click()

    privacyModal.find('input[name="limit-type"][value="0"]').parent().click()
  } else {
    privacyModal
    ...
    if (privacy.IsAlways) {
      ...
      privacyModal.find('input[name="limit-type"][value="1"]').parent().click()
    ...
    let beginDate = UTCDateToLocaleString(
      new Date(privacy.BeginDate.replace('T', ' ') + ' UTC')
    )
    let endDate = UTCDateToLocaleString(
      new Date(privacy.EndDate.replace('T', ' ') + ' UTC')
    )
    privacyModal.find('#privacy-level-begin-date').val(beginDate)

    privacyModal.find('#privacy-level-end-date').val(endDate)
  }
}
