...
export function LoadFamilyTree() {
  ...
  if (window._familyTrees.length == 0) {
    ...
    if (sessionStorage.getItem('StartFamilyTree') === null) {
      ...
      let tree = window._familyTrees.find(
        (item) => item.Id == window._currentFamilyTree.Id
      )

      if (tree == null) window._currentFamilyTree = startTree

      if (
        window._currentFamilyTree.MainPersonId == null &&
