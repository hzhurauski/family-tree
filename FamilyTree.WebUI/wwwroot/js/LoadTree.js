...
function LoadFamilyTree() {
  ...
  if (_familyTrees.length == 0) {
    ...
    if (sessionStorage.getItem('StartFamilyTree') === null) {
      ...
      if (tree == null) _currentFamilyTree = startTree

      if (
        _currentFamilyTree.MainPersonId == null &&
        tree.MainPersonId != null
      ) {
    ...
    if (_currentFamilyTree.MainPersonId == null) {
      ...
      GetFamilyTree(
        _currentFamilyTree.Id,
        _currentFamilyTree.MainPersonId
      ).then(
        (result) => {
          mainTree = result
          $('#mainPerson')[0].setAttribute(
            'data-value',
            _currentFamilyTree.MainPersonId
          )
...
function InitFamilyTreeEvents() {
  $('.person').dblclick(function (event) {
    ...
    _currentFamilyTree.MainPersonId = $(event.currentTarget)[0].getAttribute(
      'data-value'
    )
  ...
  $('#cancel-person-button').click(() => {
    ...
    if (_familyTrees.length == 0) ShowStartTree()
    else ShowMainTree()
...
function OnUpdateMainPersonButtonClick(target) {
  ...
  UpdateMainPerson(personId).then(
    (result) => {
      _familyTrees = GetFamilyTrees()
      ShowStarButtons()
    },
...
function ShowStarButtons() {
  ...
  let visiblePeopleElements = peopleElements.filter(
    (i, el) => $(el).css('visibility') === 'visible'
  )

  let hiddenPeopleElements = peopleElements.filter(
    (i, el) => $(el).css('visibility') === 'hidden'
  )

  visiblePeopleElements.children('.star-button').css('visibility', 'visible')

  hiddenPeopleElements.children('.star-button').css('visibility', 'hidden')
...
function ClearInputs() {
  ...
  $('#create-person-block')
    .find('input[name="person-gender"][value="Unknown"]')
    .parent()
    .click()
}
...
function OnCreateFamilyTreeSubmitButtonClick() {
  let treeName = $('#create-family-tree-modal').find('#family-tree-name').val()
...
function FillTree(tree, wifePart) {
  if (!wifePart) {
    // если заполнять только дерево со стороны жены, то пропустить этот блок
    ...
    var itemMarginPerson_LittleTree =
      itemSize_LittleTree - 0 + (itemMargin_LittleTree - 0) // для УМД
...
function FillPerson(person, data) {
  ...
  if (data.AvatarImageId != null) {
    person.firstElementChild.firstElementChild.src =
...
function GetPerson(person, LittleTree) {
  ...
  if (person.AvatarImage != null) {
    newImg.src =
      'data:image/' +
      person.AvatarImage.ImageFormat +
      ';base64,' +
  ...
  $(starButtonElement).click(OnUpdateMainPersonButtonClick)
...
function DrawConnections(tree, blood) {
  ...
  for (var i = 0; i < 8; i++) {
    if (tree.Grand_has_parent[i] == true) {
      if (bloodFlag)
        if (bloodTree.Grand[i]) colorLine = colorBloodMuted
  ...
  for (var i = 0; i < 4; i++) {
    if (tree.Parent_has_brother[i] == true) {
      if (bloodFlag)
        if (bloodTree.Parent[i]) colorLine = colorBloodMuted
        else colorLine = colorMuted
      else colorLine = colorMuted
  ...
  for (var i = 0; i < 8; i++) {
    if (i % 2 == 0) {
      if (arrGrand[i] != null) {
        if (bloodFlag)
          if (bloodTree.Grand[i]) colorLine = colorBlood
  ...
  for (var i = 0; i < 4; i++) {
    if (arrParent[i] != null) {
      if (arrGrand[i * 2] == null && arrGrand[i * 2 + 1] == null) {
        ...
        if (bloodFlag)
          if (bloodTree.Parent[i]) colorLine = colorBlood
          else colorLine = colorDefault
        else colorLine = colorDefault
        drawLine(
          context,
  ...
  if (tree.Brothers != null && tree.Brothers.length > 0) {
    if (bloodFlag)
      if (bloodTree.Parent[0] || bloodTree.Parent[1]) colorLine = colorBlood
      else colorLine = colorDefault
    else colorLine = colorDefault
    ...
    if (colorLine == colorBlood) colorLine = colorBloodMuted
    else colorLine = colorDefault
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if (arrParent[i * 2] == null && arrParent[i * 2 + 1] == null) {
      if (bloodFlag)
        if (bloodTree.Parent[i * 2] || bloodTree.Parent[i * 2 + 1])
  ...
  if (tree.Wife_2_has_parent) {
    if (bloodFlag)
      if (bloodTree.Wifes[1]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  if (tree.Wife_has_another_child != null) {
    for (var i = 0; i < 2; i++) {
      if (tree.Wife_has_another_child[i]) {
        if (bloodFlag)
          if (bloodTree.Wifes[i]) colorLine = colorBloodMuted
          else colorLine = colorMuted
        else colorLine = colorMuted
        drawHalf(
          context,
  ...
  if (tree.ChildWife_2 != null) {
    if (bloodFlag)
      if (bloodTree.Main) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
    ...
    if (bloodFlag)
      if (bloodTree.Main || bloodTree.Wifes[1]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  if (tree.Children == null) {
    ...
    if (bloodFlag)
      if (bloodTree.Main) colorLine = colorBlood
      else colorLine = colorDefault
    else colorLine = colorDefault
    draw(context, 270, 0, 575, 80, width, colorLine, false, dashLength, false)

    if (bloodFlag)
      if (bloodTree.Children[0]) colorLine = colorBlood
      else colorLine = colorDefault
    else colorLine = colorDefault
    ...
    if (tree.Wifes == null) {
      ...
      if (bloodFlag)
        if (bloodTree.Wifes[0]) colorLine = colorBlood
        else colorLine = colorDefault
      else colorLine = colorDefault
  ...
  if (tree.Children != null) {
    if (bloodFlag)
      if (bloodTree.Children[0]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  if (tree.Child_Another_has_sons) {
    if (bloodFlag)
      if (bloodTree.AnotherChild) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
...
function DrawConntecionsLittleTree(tree, blood) {
  ...
  if (arrGrand[0] != null || arrGrand[1] != null) {
    if (bloodFlag)
      if (bloodTree.Parent[0]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  if (arrGrand[2] != null || arrGrand[3] != null) {
    if (bloodFlag)
      if (bloodTree.Parent[1]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  for (var i = 0; i < 2; i++) {
    if (i % 2 == 0) {
      if (arrParent[i] != null) {
        if (bloodFlag)
          if (bloodTree.Parent[0]) colorLine = colorBlood
          else colorLine = colorDefault
        else colorLine = colorDefault
        draw(
          context,
      ...
      if (arrParent[i] != null) {
        if (bloodFlag)
          if (bloodTree.Parent[1]) colorLine = colorBlood
          else colorLine = colorDefault
        else colorLine = colorDefault
        draw(
          context,
  ...
  if (arrParent[2] != null || arrParent[3] != null) {
    if (bloodFlag)
      if (bloodTree.Wifes[0]) colorLine = colorBloodMuted
      else colorLine = colorMuted
    else colorLine = colorMuted
  ...
  if (tree.Children == null) {
    ...
    if (bloodFlag)
      if (bloodTree.Main || bloodTree.Wifes[0]) colorLine = colorBlood
      else colorLine = colorDefault
    else colorLine = colorDefault
  ...
  if (tree.BrothersSons != null && tree.BrothersSons.length > 0) {
    if (tree.BrothersSons[0]) {
      if (bloodFlag)
        if (bloodTree.Brothers[0]) colorLine = colorBloodMuted
        else colorLine = colorMuted
      else colorLine = colorMuted
...
function ReloadTree(personId) {
  GetFamilyTree(_currentFamilyTree.Id, personId).then(
    (result) => {
...
function ShowModalPerson(event) {
  if (
    event.currentTarget.firstElementChild.classList[1] == 'hiddenPersonContent'
  ) {
    return
  }
  ...
  $.ajax({
    ...
    success: function (result) {
      textModal.innerText =
        'Дата рождения: ' + birthday.innerText + '\n' + result
      // Дополнение: вывод даты рождения в карточке человека.
      modalPerson.firstElementChild.firstElementChild.innerText =
        surname.innerText + ' ' + name.innerText + ' ' + middleName.innerText
      // Дополнение: вывод даты рождения в карточке человека.
      var modalRect = modalPerson.getBoundingClientRect()

      modalPerson.style.left =
        Rect.left + Rect.width / 2 - modalRect.width / 2 + pageXOffset + 'px'
      modalPerson.style.top =
...
function RedrawSonsHasSonConnections(
  CurrentElem,
  VisibleCount,
  canvas,
  width,
...
function ImageAlign() {
  ...
  for (var k = 0; k < arr.length; k++) {
    for (var i = 0; i < arr[k].length; i++) {
      if (
        arr[k][i].style.visibility != 'hidden' &&
        !$(arr[k][i])
          .find('.imgBlock')[0]
          .classList.contains('hiddenPersonContent')
      ) {
...
function ChangeMarginPersonImage(imgCard) {
  ...
  if (intMarginWidth < 0) intMarginWidth = 0
  if (intMarginHeight < 0) intMarginHeight = 0
  img.style.margin =
    intMarginHeight.toString() + 'px ' + intMarginWidth.toString() + 'px'
}
