...
async function LoadPersonData(personId) {
  ...
  if (g_currentPerson == null) return false

  RefreshDataCategories()

  if (g_dataCategories.length === 0) return false
        ...
        myXhr.upload.addEventListener(
          ...
          function (e) {
            if (e.lengthComputable) {
              $('#add-image-modal #image-upload-progress').attr({
                value: e.loaded,
                max: e.total,
              })
            }
    ...
    xhr: function () {
      ...
      if (myXhr.upload) {
        myXhr.upload.addEventListener(
          ...
          function (e) {
            if (e.lengthComputable) {
              $('#add-video-modal #video-upload-progress').attr({
                value: e.loaded,
                max: e.total,
              })
            }
    ...
    xhr: function () {
      ...
      if (myXhr.upload) {
        myXhr.upload.addEventListener(
          ...
          function (e) {
            if (e.lengthComputable) {
              $('#add-audio-modal #audio-upload-progress').attr({
                value: e.loaded,
                max: e.total,
              })
            }
...
function OnSliderArrowClick() {
  ...
  let imageId = slider.find('.slick-active').attr('data-id')
...
function ShowDataBlocks(isShow = true) {
  $('#person-data-block').find('.data-blocks')[0].style.display = isShow
    ? 'block'
    : 'none'
  $('#person-data-block').find('#paste-participant-button')[0].style.display =
    isShow ? 'inline-block' : 'none'
}
...
function ShowPrivacyButton(isShow = true) {
  $('#person-data-block #edit-privacy-button').css(
    'display',
    isShow ? 'inline-block' : 'none'
  )
}
  ...
  if (dataHolder.DataHolderType == DataHolderTypes.Gender) {
    switch (dataHolder.Data) {
      case GenderTypes.Male: {
        $(dataHolderElement)
          .find('input[type="radio"][value="Male"]')
          .prop('checked', true)
        break
      }
      case GenderTypes.Female: {
        $(dataHolderElement)
          .find('input[type="radio"][value="Female"]')
          .prop('checked', true)
        break
      }
      case GenderTypes.Unknown: {
        $(dataHolderElement)
          .find('input[type="radio"][value="Unknown"]')
          .prop('checked', true)
        break
      }
...
function AddItemToImages(image) {
  ...
  $('#person-data-block').find('.images')[0].appendChild(imageElement)
}
...
function AddItemToParticipant(participant) {
  ...
  participantElement.classList.add(
    participant.IsOwner ? 'owner__item' : 'participants__item'
  )
...
function AddItemToVideos(video) {
  ...
  $('#person-data-block').find('.videos')[0].appendChild(videoElement)
}
...
function AddImageToSlider(image) {
  let slider = $('#image-carousel-modal').find('.slider')[0]
...
function CreateTextAreaDataHolderElement(dataHolder) {
  ...
  dataHolderSelectorElement.classList.replace(
    'data-holder__selector',
    'data-holder-textarea__selector'
  )

  let dataHolderTitleElement = CreateDataHolderTitleElement(dataHolder)
  dataHolderTitleElement.classList.replace(
    'data-holder__title',
    'data-holder-textarea__title'
  )

  let dataHolderDataElement = CreateDataHolderDataElement()
  dataHolderDataElement.classList.replace(
    'data-holder__data',
    'data-holder-textarea__data'
  )
  ...
  dataHolderPrivacyElement.classList.replace(
    'data-holder__privacy',
    'data-holder-textarea__privacy'
  )
...
function CreateDateDataHolderElement(dataHolder) {
  ...
  inputElement.value = dataHolder.Data
    ? moment(dataHolder.Data, getFormat(dataHolder.Data)).format().split('T')[0]
    : 0
...
function CreateGenderDataHolderElement(dataHolder) {
  ...
  dataHolderSelectorElement.classList.replace(
    'data-holder__selector',
    'data-holder-gender__selector'
  )

  let dataHolderTitleElement = CreateDataHolderTitleElement(dataHolder)
  dataHolderTitleElement.classList.replace(
    'data-holder__title',
    'data-holder-gender__title'
  )

  let dataHolderDataElement = CreateDataHolderDataElement()
  dataHolderDataElement.classList.replace(
    'data-holder__data',
    'data-holder-gender__data'
  )
...
async function DeleteSelectedDataBlocks() {
  ...
  if (dataBlocksIds.length == 0) return
