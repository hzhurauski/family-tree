...
export function RedrawSonsHasSonConnections(CurrentElem, VisibleCount, canvas, width, dashLength) {
    ...
    for (var i = 0; i < window.mainTree.Children.length; i++) {
        if (window.mainTree.Children[i].Id == $(CurrentElem.firstElementChild)[0].getAttribute("data-value")) {
            indexElem = i;
            break;
        }
    }
    ...
    for (var k = 0; k < arrHasSons.length; k++) {
        if (arrHasSons[k]) {
            if (VisibleCount == 3) {
                drawLine(window.context, 355 + k * 110, 0, 355 + k * 110, 35, width, colorMuted, true, dashLength);
            } else {
                drawLine(window.context, 460 + k * 230, 0, 460 + k * 230, 50, width, colorMuted, true, dashLength);
            }
        }
    }
}